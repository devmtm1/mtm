...
let CrmService = class CrmService {
    ...
    async assertOwnership(prospectId, user) {
        ...
        if (!prospect || prospect.commercialResponsableId !== user.id) {
            throw new common_1.NotFoundException('Prospect introuvable');
        }
    }
    async assertCommercialTarget(commercialResponsableId) {
        const targetUser = await this.prisma.user.findUnique({
            where: { id: commercialResponsableId },
            include: { roles: { include: { role: { select: { name: true } } } } },
        });
        if (!targetUser || !targetUser.isActive) {
            throw new common_1.BadRequestException('Utilisateur cible introuvable ou inactif');
        }
        ...
        if (!hasCommercialRole) {
            throw new common_1.BadRequestException("L'utilisateur cible n'a pas de rôle commercial");
        }
    }
    ...
    async findOne360(id, user) {
        ...
        const prospect = await this.prisma.prospect.findUnique({
            ...
            include: {
                ...
                activites: { orderBy: { dateEcheance: 'asc' } },
        ...
        const [relatedActivites, relatedDocuments, relatedDossiers, relatedAudits] = await Promise.all([
            relatedIds.length
    ...
    async getOptions() {
        const [pipelineStages, activiteTypes, activiteStats, priorites] = await Promise.all([
        ...
        return {
            pipelineStages: this.asOptions(pipelineStages?.value, DEFAULT_PIPELINE_STAGES),
            activiteTypes: this.asOptions(activiteTypes?.value, DEFAULT_ACTIVITE_TYPES),
            activiteStats: this.asOptions(activiteStats?.value, DEFAULT_ACTIVITE_STATS),
    ...
    async getStats(user) {
        ...
        const [pipeline, totalProspects, nouveaux, upcomingTasksCount] = await Promise.all([
    ...
    async getUpcomingTasks(user, limit = 20) {
        const isManager = this.isManager(user);
    ...
    async getTimeline(prospectId, user) {
    ...
    async assignCommercial(prospectId, commercialResponsableId, user) {
        if (!this.isManager(user)) {
            throw new common_1.BadRequestException("Seul l'encadrement commercial peut affecter un prospect");
        }
        await this.assertOwnership(prospectId, user);
    ...
    async transitionPipeline(id, nextStage, user, justification) {
        ...
        if (!isManager &&
            ...
            throw new common_1.BadRequestException('Vous ne pouvez pas faire avancer un prospect qui ne vous est pas assigné');
        }
        await this.validatePipeline(nextStage);
        if (nextStage === 'perdu' &&
            (!justification || justification.trim().length < 3)) {
            throw new common_1.BadRequestException('Une justification est obligatoire pour passer en statut "perdu"');
        }
        const prospect = await this.prisma.prospect.update({
            where: { id },
    ...
    async convertContact(contactId, commercialResponsableId, user) {
        const contact = await this.prisma.contact.findUnique({
            where: { id: contactId },
        });
        if (!contact)
            throw new common_1.NotFoundException('Contact public introuvable');
        if (commercialResponsableId && user && !this.isManager(user)) {
            throw new common_1.BadRequestException("Seul l'encadrement commercial peut affecter un prospect à un autre utilisateur");
        }
        if (commercialResponsableId) {
            const targetUser = await this.prisma.user.findUnique({
                where: { id: commercialResponsableId },
        ...
        if (existing) {
            throw new common_1.ConflictException('Un prospect existe déjà pour ce contact (email ou téléphone)');
        }
        ...
        if (dto.commercialResponsableId !== undefined) {
            if (!this.isManager(user)) {
                ...
                if (current?.commercialResponsableId !== dto.commercialResponsableId) {
                    throw new common_1.BadRequestException("Seul l'encadrement commercial peut réaffecter un prospect");
                }
            }
    ...
    async addActivite(prospectId, dto, user) {
        await this.assertOwnership(prospectId, user);
    ...
    async updateActivite(prospectId, activiteId, dto, user) {
        await this.assertOwnership(prospectId, user);
    ...
    async removeActivite(prospectId, activiteId, user) {
        await this.assertOwnership(prospectId, user);
        const activite = await this.prisma.activiteCrm.findFirst({
            where: { id: activiteId, prospectId },
        });
    ...
    async addDocument(prospectId, dto, file, user) {
        await this.assertOwnership(prospectId, user);
        this.validateFileSize(file);
        await this.validateDocumentType(dto.type);
        const uploaded = await this.cloudinary.upload(file, `mtm/crm/${prospectId}/documents`, dto.isPublic ?? false);
    ...
    async removeDocument(prospectId, documentId, user) {
        await this.assertOwnership(prospectId, user);
    ...
    async getHistory(prospectId, user) {
        await this.assertOwnership(prospectId, user);
        const [items, total] = await Promise.all([
            this.prisma.auditLog.findMany({
                where: { entityType: 'Prospect', entityId: prospectId },
    ...
    async validatePipeline(statut) {
        ...
        if (!allowed.includes(statut)) {
            throw new common_1.BadRequestException('Statut de pipeline invalide');
        }
    }
    async validateActiviteType(type) {
        const configured = await this.prisma.systemSetting.findUnique({
            where: { key: 'crm.activiteTypes' },
        });
        const allowed = Array.isArray(configured?.value)
        ...
        if (!allowed.includes(type)) {
            throw new common_1.BadRequestException("Type d'activité invalide");
        }
    }
    async validateActiviteStatut(statut) {
        const configured = await this.prisma.systemSetting.findUnique({
            where: { key: 'crm.activiteStats' },
        });
        const allowed = Array.isArray(configured?.value)
        ...
        if (!allowed.includes(statut)) {
            throw new common_1.BadRequestException("Statut d'activité invalide");
        }
    }
    async validatePriorite(priorite) {
        const configured = await this.prisma.systemSetting.findUnique({
            where: { key: 'crm.priorites' },
        });
        const allowed = Array.isArray(configured?.value)
        ...
        if (!allowed.includes(priorite)) {
            throw new common_1.BadRequestException('Priorité invalide');
        }
    }
