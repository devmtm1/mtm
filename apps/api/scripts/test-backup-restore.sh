...
#   3. Compare le nombre de tables et de lignes entre les deux
...
if [ "$ORIGINAL_TABLES" != "$RESTORED_TABLES" ]; then
  ...
  psql "$BASE_URL/postgres" -c "DROP DATABASE IF EXISTS ${TEST_DB_NAME};" >/dev/null
  exit 1
fi
...
echo ""
echo "✅ Test de restauration réussi : $ORIGINAL_TABLES tables, structure identique."
