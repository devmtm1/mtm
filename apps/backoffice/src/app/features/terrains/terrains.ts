...
import { LucideLandPlot, LucidePlus, LucideSearch } from '@lucide/angular';
...
@Component({
  ...
  imports: [ReactiveFormsModule, AgGridAngular, MatButtonModule, MatFormFieldModule, MatInputModule, MatSelectModule, LucideLandPlot, LucidePlus, LucideSearch],
...
export class Terrains implements OnInit {
  ...
  protected readonly columnDefs: ColDef<TerrainListItem>[] = [
    ...
    {
      headerName: 'Actions', flex: 1.5, minWidth: 190, sortable: false, filter: false,
      cellRenderer: (params: ICellRendererParams<TerrainListItem>) => {
        if (!params.data) return '';
        const container = document.createElement('div');
        container.className = 'terrain-row-actions';
        const detail = document.createElement('button');
        detail.type = 'button'; detail.textContent = 'Détails'; detail.title = 'Voir les détails';
        detail.addEventListener('click', () => this.openDetail(params.data!.id));
        container.appendChild(detail);
        if (this.canModify()) {
          const edit = document.createElement('button');
          edit.type = 'button'; edit.textContent = 'Modifier'; edit.title = 'Modifier le terrain';
